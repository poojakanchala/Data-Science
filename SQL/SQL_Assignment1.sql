...
from sh.customers 
where cust_marital_status = 'Married' and cust_income_level = 'Medium'
...
select cust_first_name || ' ' || cust_last_name, cust_valid
from sh.customers
where cust_valid = 'A'
...
from sh.customers
where cust_eff_from > TO_DATE('2020-12-31', 'YYYY-MM-DD')

-- 19. Retrieve customers whose effective end date (`CUST_EFF_TO`) is before 2021.

select cust_first_name || ' ' || cust_last_name, cust_eff_to 
from sh.customers
...
select cust_first_name || ' ' || cust_last_name, cust_credit_limit
from sh.customers
where cust_credit_limit>5000 and cust_credit_limit<9000
